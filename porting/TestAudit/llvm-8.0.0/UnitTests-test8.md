...
- **Golden file:** ans-inst.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test8/Makefile`)
- **Criterion:** (none — CRITERION empty, whole-program default)
- **EXPECTED_EXIT:** 54   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (4 lines: 6, 12, 18, 24)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test8/README.md` (3.4-era, unchanged). Program compiled with
...
`EXPECTED_EXIT=54` (or unchecked, `EXIT_UNCHECKED=0`); no
