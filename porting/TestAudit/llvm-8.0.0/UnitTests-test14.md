...
- **Golden file:** ans-inst.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test14/Makefile`)
- **Criterion:** (none — CRITERION empty, whole-program default)
- **EXPECTED_EXIT:** 2   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (7 lines: 13, 20, 27, 40, 42, 44, 47)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test14/README.md` (3.4-era, unchanged). Program compiled with
...
`EXPECTED_EXIT=2` (or unchecked, `EXIT_UNCHECKED=0`); no
