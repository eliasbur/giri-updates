...
- **Golden file:** ans-inst.txt / ans-loc.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test17/Makefile`)
- **Criterion:** criterion-inst.txt / criterion-loc.txt
- **EXPECTED_EXIT:** -1   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (6 lines: 11, 16, 21, 25, 26, 27)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test17/README.md` (3.4-era, unchanged). Program compiled with
