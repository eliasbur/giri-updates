...
- **Golden file:** ans-inst-seq.txt   **Input:** (see per-test `INPUT` in `test/kmeans/Makefile`)
- **Criterion:** criterion-inst-seq.txt
- **EXPECTED_EXIT:** -1   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (2 lines: 222, 276)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/kmeans/README.md` (3.4-era, unchanged). Program compiled with
