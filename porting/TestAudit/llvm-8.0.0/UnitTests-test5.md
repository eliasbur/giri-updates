...
- **Golden file:** ans-inst.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test5/Makefile`)
- **Criterion:** (none — CRITERION empty, whole-program default)
- **EXPECTED_EXIT:** -1   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (12 lines: 23, 28, 29, 30, 32, 34, 35, 39, 40, 41, 45, 46)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test5/README.md` (3.4-era, unchanged). Program compiled with
