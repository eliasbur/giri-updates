...
- **Golden file:** ans-inst.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test19/Makefile`)
- **Criterion:** (none — CRITERION empty, whole-program default)
- **EXPECTED_EXIT:** 13   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (10 lines: 6, 8, 9, 11, 12, 19, 24, 25, 28, 29)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test19/README.md` (3.4-era, unchanged). Program compiled with
...
`EXPECTED_EXIT=13` (or unchecked, `EXIT_UNCHECKED=0`); no
