...
- **Golden file:** ans-inst.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test10/Makefile`)
- **Criterion:** (none — CRITERION empty, whole-program default)
- **EXPECTED_EXIT:** 103   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (4 lines: 13, 18, 19, 28)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test10/README.md` (3.4-era, unchanged). Program compiled with
...
`EXPECTED_EXIT=103` (or unchecked, `EXIT_UNCHECKED=0`); no
