...
- **Golden file:** ans-inst.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test15/Makefile`)
- **Criterion:** (none — CRITERION empty, whole-program default)
- **EXPECTED_EXIT:** 31   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (12 lines: 6, 7, 8, 10, 11, 12, 13, 20, 25, 26, 28, 29)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test15/README.md` (3.4-era, unchanged). Program compiled with
...
`EXPECTED_EXIT=31` (or unchecked, `EXIT_UNCHECKED=0`); no
