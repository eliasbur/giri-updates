...
- **Golden file:** ans-inst.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test3/Makefile`)
- **Criterion:** (none — CRITERION empty, whole-program default)
- **EXPECTED_EXIT:** 98   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (9 lines: 7, 8, 9, 12, 13, 17, 22, 23, 26)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test3/README.md` (3.4-era, unchanged). Program compiled with
...
`EXPECTED_EXIT=98` (or unchecked, `EXIT_UNCHECKED=0`); no
