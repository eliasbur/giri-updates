...
- **Golden file:** ans-inst.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test12/Makefile`)
- **Criterion:** (none — CRITERION empty, whole-program default)
- **EXPECTED_EXIT:** 28   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (20 lines: 15, 20, 25, 26, 27, 28, 29, 32, 33, 40, 41, 44, 51, 56, 57, 58, 59, 61, 62, 64)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test12/README.md` (3.4-era, unchanged). Program compiled with
...
`EXPECTED_EXIT=28` (or unchecked, `EXIT_UNCHECKED=0`); no
