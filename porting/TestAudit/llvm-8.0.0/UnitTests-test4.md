...
- **Golden file:** ans-inst.txt / ans-loc.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test4/Makefile`)
- **Criterion:** criterion-inst.txt / criterion-loc.txt
- **EXPECTED_EXIT:** 5   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (7 lines: 9, 11, 16, 21, 22, 26, 27)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test4/README.md` (3.4-era, unchanged). Program compiled with
...
`EXPECTED_EXIT=5` (or unchecked, `EXIT_UNCHECKED=0`); no
