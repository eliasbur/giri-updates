...
- **Golden file:** ans-inst.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test20/Makefile`)
- **Criterion:** (none — CRITERION empty, whole-program default)
- **EXPECTED_EXIT:** -1   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (11 lines: 5, 8, 9, 12, 13, 15, 16, 22, 27, 29, 30)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test20/README.md` (3.4-era, unchanged). Program compiled with
