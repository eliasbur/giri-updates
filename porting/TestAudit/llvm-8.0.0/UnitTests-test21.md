...
- **Golden file:** ans-inst.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test21/Makefile`)
- **Criterion:** criterion-inst.txt
- **EXPECTED_EXIT:** -1   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (6 lines: 4, 11, 18, 23, 24, 25)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test21/README.md` (3.4-era, unchanged). Program compiled with
