...
- **Golden file:** ans-inst.txt   **Input:** (see per-test `INPUT` in `test/UnitTests/test9/Makefile`)
- **Criterion:** (none — CRITERION empty, whole-program default)
- **EXPECTED_EXIT:** -1   **EXIT_UNCHECKED:** 1
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (5 lines: 9, 14, 15, 20, 25)
- **Root cause:** none — test passes cleanly on LLVM 8.0.0

## What the test does
See `test/UnitTests/test9/README.md` (3.4-era, unchanged). Program compiled with
...
`EXPECTED_EXIT=-1` (or unchecked, `EXIT_UNCHECKED=1`); no
