...
- **Criterion:** `criterion-inst=criterion-inst.txt` (`main 51`)
...
## Stage-by-stage output (honest harness, per-test `_test_logs/UnitTests_test21.log`)
...
No `[GIRI] Abnormal termination` marker. instruction-number criterion; `Start slicing Function:Instruction` message; no `[GIRI] Abnormal` marker.
