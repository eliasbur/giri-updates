...
- **Golden file:** ans-inst.txt (**5** source-line entries)   **Pristine:** yes (no `ans-*.txt` change; `git diff 224bdfb..HEAD -- test/` is empty)
- **Criterion:** none (CRITERION empty; `EXIT_UNCHECKED=1`)
...
## Stage-by-stage output (honest harness, per-test `_test_logs/UnitTests_test9.log`)
...
No `[GIRI] Abnormal termination` marker. exit unchecked (UB/pthread-independent iteration); `The min is 6, and the max is 302`; no `[GIRI] Abnormal` marker.
