...
- **Golden file:** ans-inst.txt (**4** source-line entries)   **Pristine:** yes (no `ans-*.txt` change; `git diff 224bdfb..HEAD -- test/` is empty)
...
## Stage-by-stage output (honest harness, per-test `_test_logs/UnitTests_test1.log`)
...
No `[GIRI] Abnormal termination` marker. no `[GIRI] Abnormal` marker; program output `MingliangLIU` / `12` (EXPECTED_EXIT=12).
