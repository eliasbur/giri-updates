...
- **Golden file:** ans-inst.txt (**7** source-line entries)   **Pristine:** yes (no `ans-*.txt` change; `git diff 224bdfb..HEAD -- test/` is empty)
- **Criterion:** `criterion-loc=criterion-loc.txt` (two lines: `example.c 26`, `example.c 27`)
...
## Stage-by-stage output (honest harness, per-test `_test_logs/UnitTests_test4.log`)
...
No `[GIRI] Abnormal termination` marker. two source-line criteria; `Start slicing File:Line` messages per criterion; no `[GIRI] Abnormal` marker.
