...
- **Golden file:** ans-inst.txt (**11** source-line entries)   **Pristine:** yes (no `ans-*.txt` change; `git diff 224bdfb..HEAD -- test/` is empty)
...
## Stage-by-stage output (honest harness, per-test `_test_logs/UnitTests_test20.log`)
...
No `[GIRI] Abnormal termination` marker. a benign `implicit declaration` warning at compile (pre-existing, unchanged); no `[GIRI] Abnormal` marker.
