...
- **Golden file:** ans-inst.txt (**10** source-line entries)   **Pristine:** yes (no `ans-*.txt` change; `git diff 224bdfb..HEAD -- test/` is empty)
...
## Stage-by-stage output (honest harness, per-test `_test_logs/UnitTests_test19.log`)
...
No `[GIRI] Abnormal termination` marker. EXPECTED_EXIT=13; no `[GIRI] Abnormal` marker.
