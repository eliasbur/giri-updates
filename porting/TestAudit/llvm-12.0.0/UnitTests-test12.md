...
- **Golden file:** ans-inst.txt (**20** source-line entries)   **Pristine:** yes (no `ans-*.txt` change; `git diff 224bdfb..HEAD -- test/` is empty)
...
## Stage-by-stage output (honest harness, per-test `_test_logs/UnitTests_test12.log`)
...
For some variable length functions like ap_rprintf in apache, call records missing. Stop here for now. Fix it later
i8* %0
For some variable leng
```

No `[GIRI] Abnormal termination` marker. pthread test; `The supposed sum is: 32640`; EXPECTED_EXIT=28; no `[GIRI] Abnormal` marker.
