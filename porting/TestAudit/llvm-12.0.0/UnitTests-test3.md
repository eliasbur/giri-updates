...
- **Golden file:** ans-inst.txt (**9** source-line entries)   **Pristine:** yes (no `ans-*.txt` change; `git diff 224bdfb..HEAD -- test/` is empty)
...
## Stage-by-stage output (honest harness, per-test `_test_logs/UnitTests_test3.log`)
...
No `[GIRI] Abnormal termination` marker. program prints `fibonacci(15) is 610` (EXPECTED_EXIT=98); no `[GIRI] Abnormal` marker.
