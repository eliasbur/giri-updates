...
## Stage-by-stage output (honest harness, per-test `_test_logs/UnitTests_test2.log`)
...
No `[GIRI] Abnormal termination` marker. program prints the `a b c d` branch result; no `[GIRI] Abnormal` marker.
