...
- **Golden file:** ans-inst-seq.txt (**19** source-line entries)   **Pristine:** yes (no `ans-*.txt` change; `git diff 224bdfb..HEAD -- test/` is empty)
- **Criterion:** `criterion-inst=criterion-inst-seq.txt` (`matrix_mult 291`)
...
## Stage-by-stage output (honest harness, per-test `_test_logs/matrix_multiply.log`)
...
No `[GIRI] Abnormal termination` marker. seq variant (LDFLAGS cleared); `MatrixMult: Side of the matrix is 4`; 19-line slice matches the 3.4 golden exactly.
