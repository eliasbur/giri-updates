# test19 (UnitTests/test19)

- **Verdict:** CLEAN
- **Golden file:** ans-inst.txt (10 lines)   **Input:** (see per-test `INPUT` in `test/UnitTests/test19/Makefile`)
...
## What the test does
See `test/UnitTests/test19/README.md` (3.4-era, unchanged). Program (`fibonacci.c`) compiled with
...
evidence run log (`test/_test_logs/test19.log`, commit 74b870f):

```
Program stdout: (silent run — no output)
```