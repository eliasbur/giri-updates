# struct (test/UnitTests/test13)

- **Verdict:** CLEAN
- **Golden file:** ans-inst.txt (6 lines)   **Input:** 10
...
## What the test does
Program (`struct`) compiled with `clang -g -O0 -c -emit-llvm -Xclang -no-opaque-pointers`
...
`_test_logs/test13.log` (see the per-test log under `_test_logs/`).
