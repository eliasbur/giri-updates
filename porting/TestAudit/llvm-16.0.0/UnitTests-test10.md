# str (test/UnitTests/test10)

- **Verdict:** CLEAN
- **Golden file:** ans-inst.txt (4 lines)   **Input:** giri
- **Criterion:** none (slices at the implicit criterion)
- **EXPECTED_EXIT:** 103   **EXIT_UNCHECKED:** 0
...
## What the test does
Program (`str`) compiled with `clang -g -O0 -c -emit-llvm -Xclang -no-opaque-pointers`
...
`_test_logs/test10.log` (see the per-test log under `_test_logs/`).
