...
- **Verdict:** CLEAN
- **Golden file:** ans-inst.txt (7 lines)   **Input:** 10
- **Criterion:** `criterion-loc`
...
## What the test does
Program (`example`) compiled with `clang -g -O0 -c -emit-llvm -Xclang -no-opaque-pointers`
...
`_test_logs/test4.log` (see the per-test log under `_test_logs/`).
