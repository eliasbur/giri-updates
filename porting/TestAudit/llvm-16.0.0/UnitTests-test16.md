# struct-ptr (test/UnitTests/test16)
...
## What the test does
Program (`struct-ptr`) compiled with `clang -g -O0 -c -emit-llvm -Xclang -no-opaque-pointers`
...
`_test_logs/test16.log` (see the per-test log under `_test_logs/`).
