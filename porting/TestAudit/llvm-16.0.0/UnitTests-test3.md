# fibonacci (test/UnitTests/test3)

- **Verdict:** CLEAN
- **Golden file:** ans-inst.txt (9 lines)   **Input:** 15
- **Criterion:** none (slices at the implicit criterion)
- **EXPECTED_EXIT:** 98   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to the pristine 3.4-era golden (the harness `diff` exits 0; independently re-verified by the standalone-`tracer` validation: 9 slice locs == golden, prtrace OK)
- **Root cause:** none — test passes cleanly on LLVM 16.0.0 (new pass manager)

## What the test does
Program (`fibonacci`) compiled with `clang -g -O0 -c -emit-llvm -Xclang -no-opaque-pointers`
...
`_test_logs/test3.log` (see the per-test log under `_test_logs/`).
