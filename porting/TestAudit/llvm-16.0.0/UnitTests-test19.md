# fibocci (test/UnitTests/test19)

- **Verdict:** CLEAN
- **Golden file:** ans-inst.txt (10 lines)   **Input:** 7
- **Criterion:** none (slices at the implicit criterion)
- **EXPECTED_EXIT:** 13   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to the pristine 3.4-era golden (the harness `diff` exits 0; independently re-verified by the standalone-`tracer` validation: 10 slice locs == golden, prtrace OK)
- **Root cause:** none — test passes cleanly on LLVM 16.0.0 (new pass manager)

## What the test does
Program (`fibocci`) compiled with `clang -g -O0 -c -emit-llvm -Xclang -no-opaque-pointers`
...
`_test_logs/test19.log` (see the per-test log under `_test_logs/`).
