# matrix_multiply-seq (test/matrix_multiply)

- **Verdict:** CLEAN
- **Golden file:** ans-inst-seq.txt (19 lines)   **Input:** <seq>
- **Criterion:** criterion-inst-seq.txt
- **EXPECTED_EXIT:** 0   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to the pristine 3.4-era golden (the harness `diff` exits 0; independently re-verified by the standalone-`tracer` validation: 19 slice locs == golden, prtrace OK)
- **Root cause:** none — test passes cleanly on LLVM 16.0.0 (new pass manager)

## What the test does
Program (`matrix_multiply-seq`) compiled with `clang -g -O0 -c -emit-llvm -Xclang -no-opaque-pointers`
(clang/LLVM 16.0.0), linked (`llvm-link`), instrumented with the `trace-giri` pass under
**new pass manager** (`opt -passes="function(mergereturn),bbnum,lsnum,trace-giri,remove-bbnum,remove-lsnum"`, seq variant, each Giri library loaded via `-load` + `-load-pass-plugin`), traced, then sliced with `dgiri` in the same pipeline against the criterion; the
...
`_test_logs/matrix_multiply.log` (see the per-test log under `_test_logs/`).
