# forloop (test/UnitTests/test9)

- **Verdict:** CLEAN
- **Golden file:** ans-inst.txt (5 lines)   **Input:** 6 19 7 17 32 35 40 54 302
- **Criterion:** none (slices at the implicit criterion)
- **EXPECTED_EXIT:** <unchecked>   **EXIT_UNCHECKED:** 1
- **Diff:** empty — `$(NAME).slice.loc` identical to the pristine 3.4-era golden (the harness `diff` exits 0; independently re-verified by the standalone-`tracer` validation: 5 slice locs == golden, prtrace OK)
- **Root cause:** none — test passes cleanly on LLVM 16.0.0 (new pass manager)

## What the test does
Program (`forloop`) compiled with `clang -g -O0 -c -emit-llvm -Xclang -no-opaque-pointers`
...
`_test_logs/test9.log` (see the per-test log under `_test_logs/`).
