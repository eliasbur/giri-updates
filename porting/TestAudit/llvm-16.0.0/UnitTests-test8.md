# ptr (test/UnitTests/test8)

- **Verdict:** CLEAN
- **Golden file:** ans-inst.txt (4 lines)   **Input:** 40 54
- **Criterion:** none (slices at the implicit criterion)
- **EXPECTED_EXIT:** 54   **EXIT_UNCHECKED:** 0
...
## What the test does
Program (`ptr`) compiled with `clang -g -O0 -c -emit-llvm -Xclang -no-opaque-pointers`
...
`_test_logs/test8.log` (see the per-test log under `_test_logs/`).
