...
- **Verdict:** CLEAN
- **Golden file:** ans-inst.txt (6 lines)   **Input:** 4
...
## What the test does
Program (`plower`) compiled with `clang -g -O0 -c -emit-llvm -Xclang -no-opaque-pointers`
...
`_test_logs/test17.log` (see the per-test log under `_test_logs/`).
