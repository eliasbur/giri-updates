...
- **Verdict:** CLEAN
- **Golden file:** ans-inst-seq.txt (17 lines)   **Input:** <seq>
- **Criterion:** criterion-inst-seq.txt
- **EXPECTED_EXIT:** 0   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to the pristine 3.4-era golden (the harness `diff` exits 0; independently re-verified by the standalone-`tracer` validation: 17 slice locs == golden, prtrace OK)
- **Root cause:** none — test passes cleanly on LLVM 16.0.0 (new pass manager)

## What the test does
Program (`pca-seq`) compiled with `clang -g -O0 -c -emit-llvm -Xclang -no-opaque-pointers`
...
`_test_logs/pca.log` (see the per-test log under `_test_logs/`).
