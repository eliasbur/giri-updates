...
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (the harness `diff` exits 0; independently re-verified by the standalone-`tracer` validation: 10 slice locs == golden, prtrace OK)
- **Root cause:** none — test passes cleanly on LLVM 15.0.0 (new pass manager)

## What the test does
Program (`fibocci`) compiled with `clang -g -O0 -c -emit-llvm` (clang/LLVM 15.0.0),
...
`_test_logs/UnitTests_test19.log`:
