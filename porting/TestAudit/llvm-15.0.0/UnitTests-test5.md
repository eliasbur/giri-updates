# hellothreads (test/UnitTests/test5)

- **Verdict:** CLEAN
- **Golden file:** ans-inst.txt (12 lines)   **Input:** 8
- **Criterion:** none (slices at the implicit criterion)
- **EXPECTED_EXIT:** -1   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (the harness `diff` exits 0; independently re-verified by the standalone-`tracer` validation: 12 slice locs == golden, prtrace OK)
- **Root cause:** none — test passes cleanly on LLVM 15.0.0 (new pass manager)

## What the test does
Program (`hellothreads`) compiled with `clang -g -O0 -c -emit-llvm` (clang/LLVM 15.0.0),
...
`_test_logs/UnitTests_test5.log`:

```
  [0] Hello thread 140380025460480!
  [1] Hello thread 140380017067776!
  [2] Hello thread 140380008675072!
  [3] Hello thread 140379932325632!
  [4] Hello thread 140379923932928!
  [5] Hello thread 140379915540224!
  [6] Hello thread 140379907147520!
  [7] Hello thread 140379898754816!
```
