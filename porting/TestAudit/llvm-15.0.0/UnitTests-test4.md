# example (test/UnitTests/test4)

- **Verdict:** CLEAN
- **Golden file:** ans-inst.txt (7 lines; == ans-loc.txt)   **Input:** 10
- **Criterion:** -criterion-loc=criterion-loc.txt
- **EXPECTED_EXIT:** 5   **EXIT_UNCHECKED:** 0
- **Diff:** empty — `$(NAME).slice.loc` identical to golden (the harness `diff` exits 0; independently re-verified by the standalone-`tracer` validation: 7 slice locs == golden, prtrace OK)
- **Root cause:** none — test passes cleanly on LLVM 15.0.0 (new pass manager)

## What the test does
Program (`example`) compiled with `clang -g -O0 -c -emit-llvm` (clang/LLVM 15.0.0),
...
`_test_logs/UnitTests_test4.log`:
...
exits 0 (PASS); the slicing stage prints `Start slicing Filename:Loc is defined as example.c:26`; and no `[GIRI] Abnormal termination` line appears in the captured log.
