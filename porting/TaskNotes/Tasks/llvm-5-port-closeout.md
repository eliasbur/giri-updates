...
- `c9b0a12` — Parts 0+1: verified crash detection (injected `raise(SIGSEGV)` in test2, confirmed `[GIRI] Abnormal termination` marker causes `[FAIL]` at trace stage, verified no stray `tmperr` or `/tmp` leak, full suite 21 PASS / 1 FAIL at commit `d1add98`). Verified invariant 1 (BB/LS numbering deterministic across runs, test2 single-file: 5 BBs/20 LS, test16 multi-file: 8 BBs, repeat runs agree). Verified invariant 2 (`Runtime.h` unchanged against `port/llvm-5.0.2`, `sizeof(Entry)`=32 divides PAGESIZE 4096 → 128 entries/page). Verified invariant 3 (`-srcline-mapping` on test2: mapped `ifelse.c` lines 4-14 correctly, `NIL` for no-debug-info instructions). Verified all three invariants hold. Next: Parts 2-4 documentation reconciliation.
- `d1add98` — Parts 2-4: ticked all 13 `llvm-5-port.md` boxes (10 deferred api-breakings box left unticked with pointer); rewrote `llvm-5-test-fixes.md` stale boxes (kmeans settled; PR #7 merged as 3b26ea6); reconciled `SUMMARY.md` (root cause A: 10 tests, not 8; replaced scratch work with clean reconciliation block; five suite results reconciled; FAIL-EXPECTED: documented as accepted end state in AGENTS.md; test6/test7/test22: added note to `auto-tests.txt` header; added no-asserts note to AgentGuide.md; recorded code observations for `ensurePostDomFrontierComputed` leak and `properlyDominates` overload). Next: open PR.
